...
{
	[CmdletBinding()]
	param( [Parameter( ValueFromPipeline=$true )] $settings )
	...
	write-verbose "$debugpreference,$verbosepreference";
	
	if( ( 'inquire' -eq $debugpreference ) -or 'none' -ne $settings.startupLogLevel )
	{
	...
	if( 'continue' -eq $verbosepreference -or ( 'verbose' -eq $settings.startupLogLevel ))
	{
